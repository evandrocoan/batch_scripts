...
set "GOODSYNC_DIRECTORY=%UserProfile%\Downloads\GoodSync"


if exist "%GOODSYNC_DIRECTORY%" goto :end

SET "ORIGINAL=%CD%"

:: https://stackoverflow.com/questions/9049470/batch-to-open-a-folder-within-a-users-folderc-users-usernamehere-my-documents
"%ORIGINAL%\Programs\wrar\WinRAR.exe" x -o+ "%ORIGINAL%\Programs\GoodSync.7z" "%UserProfile%\Downloads\"
...
if defined PROGRAMFILES(X86) (
    ...
    if exist "%GOODSYNC_DIRECTORY%\x64\GoodSync-v10.exe" (
        start /max "" "%GOODSYNC_DIRECTORY%\x64\GoodSync-v10.exe"
    ) else (
        start /max "" "%GOODSYNC_DIRECTORY%\x86\GoodSync2Go-v10.exe"
    )
) else (
    echo Running GoodSync2Go on 32-bit system
    rem start /max "" "%GOODSYNC_DIRECTORY%\x86\GoodSync2Go-v10.exe"
    start /max "" "%GOODSYNC_DIRECTORY%\x86\GoodSync-v10.exe"
)
...
rem pause
title GoodSync Setup - Took %hh%:%mm%:%ss%,%cc% seconds to run this script
timeout /T 60
endlocal
