...
if exist "%CYGWIN_PORTABLE_DIRECTORY%\Cygwin\usr\local\bin\sshpass.exe" goto :end

if "%1"=="" SET "ORIGINAL=%CD%"

if not "%1"=="" SET "ORIGINAL=%1"

set "SSHPASS_DIRECTORY=%ORIGINAL%\Programs\sshpass"

"%CYGWIN_PORTABLE_DIRECTORY%\Cygwin\bin\mintty.exe" -w max -h always -e /bin/bash --login -i -c "cd $(cygpath -u '%SSHPASS_DIRECTORY%'); make install"
...
rem pause
title sshpass Setup - Took %hh%:%mm%:%ss%,%cc% seconds to run this script
if "%1"=="" timeout /T 60
endlocal
