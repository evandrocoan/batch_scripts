...
2. **⚫ Canal desligado (ON solto):** Antes de ajustar o ganho, **mantenha o canal desligado** (botão ON solto/não apertado). Isso garante que nenhum som saia nas caixas enquanto você calibra o sinal. O botão **ON** só deve ser apertado no passo 10, depois que o ganho estiver calibrado e você estiver pronto para abrir o fader.
...
| **Sinal verde acende mas silêncio nas caixas** | Botão **ST** e fader vermelho | O sinal está chegando ao canal, mas não foi roteado. Verifique se o botão **ST** do canal está **APERTADO** e se o **fader vermelho (Master ST)** está levantado. |
| **Músico ouve no retorno mas o povo não ouve nada** | Botão **ST** | O botão **ST** do canal está **SOLTO**. O sinal vai para os retornos (AUX 1/2 são PRE-fader) mas não chega às caixas da assembleia. Aperte o botão **ST**. |
| **Som "Rachando"** | Luz Vermelha (**PEAK**) | O som está entrando muito forte. Abaixe o botão **GAIN** (lá em cima). |
| **Volume dobrado / distorção estranha sem causa** | Botões **ST** e **1-2 / 3-4** do canal | O canal está com **ST** e **1-2** (ou 3-4) apertados ao mesmo tempo, e o fader do grupo também tem ST ativo — o sinal chega às caixas por dois caminhos, dobrando o volume. Escolha **apenas um caminho**: ou ST direto, ou grupo com ST no fader do grupo. |
| **Microfonia (Apito)** | Retornos (**AUX 1/2**) | Abaixe um pouco o botão azul **AUX 1 ou 2** do microfone que está apitando. |
| **Músico não se ouve (retorno do altar)** | Botões **AUX 1/2**, **PRE**, master AUX 1, caixa física e cabo | Verifique se o botão **PRE** está **APERTADO** e se o botão azul **AUX 1** do canal está girado para a direita. Verifique também se o **controle master geral do AUX 1** (botão giratório no canto superior direito da mesa) não está zerado. Se nada resolver: a caixa do AUX 1 é ligada na tomada e possui botão de **power** e botão de **volume** próprios — músicos já desligaram ou zeraram o volume dela sem perceber. Confira também se o **cabo da saída AUX 1 da mesa até a caixa** não foi desconectado. |
| **Músico não se ouve (retorno da bateria)** | Botões **AUX 2**, **PRE**, master AUX 2 e cabo | Verifique se o botão **PRE** está **APERTADO**, se o botão azul **AUX 2** do canal está girado para a direita, se o **controle master geral do AUX 2** (botão giratório no canto superior direito da mesa) não está zerado e se o **cabo da saída AUX 2 da mesa até a caixa de retorno** não foi desconectado. |
| **Caixas laterais sem som, caixas grandes funcionam** | Botão **PRE inferior**, master AUX 4 e fader | O **botão PRE inferior** (entre AUX 4 e AUX 5/6) pode estar **APERTADO** acidentalmente, tornando o AUX 4 independente do fader. Mantenha-o **SOLTO**. Verifique também se o botão azul **AUX 4** do canal está girado para a direita e se o **controle master geral do AUX 4** (botão giratório no canto superior direito da mesa) não está zerado. |
| **Som abafado / como dentro d'água** | Botão **LPF** na saída MONO | O botão **LPF** (*Low Pass Filter*) da saída MONO está ativado, cortando agudos e médios. Verifique se a gravação/transmissão está usando a saída MONO por engano — **use ST SUB OUT**. Se for necessário usar MONO, desaperte o botão LPF. |
...
| **Estouro/estalo ao ligar ou desligar** | **Ordem de ligar/desligar** | A ordem foi invertida. **Para ligar:** baixe os faders mestres → ligue a mesa → ligue os amplificadores/caixas → suba os faders. **Para desligar:** baixe os faders → desligue os amplificadores/caixas primeiro → depois desligue a mesa. Nunca inverta essa ordem — transientes podem queimar os alto-falantes. |
| **Som sumiu ao usar grupos (1-2 ou 3-4)** | Botão **ST** no fader do grupo | O canal foi roteado para o grupo (botão **1-2** ou **3-4** apertado), mas o botão **ST** no **fader do grupo** está **SOLTO**. Aperte o botão **ST** no fader do grupo correspondente para que o som saia nas caixas. Sem isso, o som fica preso no grupo e não vai para lugar nenhum. |
| **Gravação via AUX 3 faltando instrumento** | Botão azul **AUX 3** do canal | Se a gravação/transmissão usa a saída AUX 3 (mixagem personalizada), cada canal precisa ter o botão azul **AUX 3** girado para a direita. Se um novo canal foi ligado e o operador esqueceu de girar o AUX 3 daquele canal, ele **não entra na gravação**. Verifique todos os canais ativos. |
...
1. **Celebrante** — O padre ou diácono que preside a celebração litúrgica. Normalmente usa microfone no ambão (púlpito) e/ou microfone de lapela.
1. **Celebração** — A missa ou outra cerimônia litúrgica (batizado, casamento, adoração, etc.). Neste guia, é o momento em que o sistema de som precisa estar funcionando corretamente para servir a liturgia.
1. **Comentarista** — Veja *Monitor (liturgia)*.
1. **Monitor (liturgia)** — Pessoa que sobe no ambão antes do padre, faz a acolhida, lê as intenções e guia a assembleia nos momentos de sentar ou levantar. O nome vem do latim *monere* ("avisar", "instruir", "guiar") — a mesma raiz de **Monição**, que é o nome oficial dado nos livros litúrgicos (como o Missal Romano) aos textos de acolhida e explicação lidos antes da missa, antes das leituras ou antes do ofertório. Logo, quem lê a **Monição** é o **Monitor**. A Instrução Geral do Missal Romano (IGMR, n. 105b) descreve essa função: *"Cabe-lhe propor aos fiéis, brevemente, explicações e exortações para introduzi-los na celebração e dispô-los a entendê-la melhor."* No Brasil, o termo Monitor se popularizou no lugar de "Comentarista" (usado no documento original em latim) para evitar confusão com comentaristas esportivos — o Monitor fala pouco, apenas para guiar. Veja também *Retorno* nos termos de áudio para entender a ambiguidade da palavra na prática.
1. **Coral** — Grupo de cantores que atua na liturgia, geralmente posicionado próximo ao altar/palco. Pode usar microfones condensadores ("fininhos pendurados") para captação coletiva ou microfones individuais.
...
1. **Leitor** — Pessoa responsável por proclamar as leituras bíblicas durante a celebração. Normalmente usa o microfone fixo do ambão (púlpito). A variação de volume entre diferentes leitores é um dos principais desafios da operação de som.
...
1. **Missa** — A principal celebração litúrgica católica. Neste guia, é usada como referência genérica para qualquer celebração em que o sistema de som esteja em uso.
...
1. **Sacristia** — Sala anexa à igreja onde ficam guardados os paramentos, objetos litúrgicos e onde o celebrante se prepara antes da celebração. É diferente da nave (onde ficam os fiéis) e do presbitério (onde fica o altar).
...
1. **Alto-falante** — O componente interno de uma caixa de som que converte o sinal elétrico em som audível. Transientes (picos elétricos) e distorção prolongada podem danificá-lo permanentemente, por isso existe a "Ordem Sagrada de Ligar e Desligar".
1. **Amplificador** — Equipamento que recebe o sinal de áudio da mesa e o amplifica (torna mais forte) para alimentar as caixas de som. Na nossa instalação, os amplificadores ficam entre a mesa e as caixas grandes. Devem ser ligados *depois* da mesa e desligados *antes* dela.
...
1. **Compressor** — Equipamento de áudio que reduz automaticamente a diferença entre os sons mais fortes e os mais fracos de um sinal. Em termos práticos, ele "segura" os picos de quem fala alto e levanta quem fala baixo, como um operador invisível ajustando o volume o tempo todo. Útil em microfones de ambão/púlpito onde várias pessoas com volumes de voz diferentes revezam no mesmo microfone.
...
1. **Desbalanceado (cabo/sinal)** — Tipo de cabo ou conexão elétrica que usa apenas 2 fios (sinal + terra), sem proteção contra interferências. Exemplos: cabos RCA (vermelho e branco) e cabos P10 de dois contatos. Suscetível a chiados e ruídos em cabos longos ou em ambientes com equipamentos elétricos próximos. Evite para distâncias maiores que 3 metros.
...
1. **Efeito de proximidade** — Fenômeno acústico em que as frequências graves ficam artificialmente reforçadas quando o locutor fala muito perto do microfone. A voz fica "embolada" e com excesso de peso. O filtro HPF (botão 80 Hz) ajuda a compensar esse efeito.
...
1. **Microfonia / Apito** — O chiado agudo e irritante que ocorre quando o som de uma caixa é captado por um microfone próximo, criando um ciclo de retroalimentação (feedback). É o "apito" que todo mundo já ouviu numa celebração.
1. **Mixagem** — O processo de combinar (misturar) os sinais de vários microfones e instrumentos em um único som equilibrado. É o trabalho principal do operador de som: ajustar volumes, equalização e balanço de cada canal para que o resultado final soe agradável e inteligível.
1. **Retorno / Monitor (áudio)** — Caixa de som virada para os músicos, o padre ou o coro, para que eles se ouçam durante a execução. É diferente das caixas voltadas para a assembleia. Em inglês e nos manuais técnicos, é chamado de *monitor* — o que pode gerar confusão, pois na liturgia brasileira a mesma palavra designa a pessoa que faz a acolhida (veja *Monitor (liturgia)* nos termos litúrgicos). **Como não confundir na prática:** Se alguém disser *"o microfone do monitor está baixo"*, provavelmente estão falando do microfone da **pessoa** que faz a acolhida. Se alguém disser *"aumenta o meu monitor"* ou *"aumenta o monitor do padre"*, estão pedindo para aumentar o volume da **caixa de retorno** no chão, para que eles se ouçam melhor.
...
1. **Pressão sonora** — A intensidade física do som no ar, medida em dB SPL (Sound Pressure Level). Microfones dinâmicos aguentam alta pressão sonora (instrumentos de percussão, amplificadores de guitarra), enquanto microfones condensadores são mais sensíveis e podem distorcer com fontes muito intensas.
1. **Rack** — Estrutura metálica padronizada (formato 19 polegadas) usada para montar equipamentos de áudio empilhados, como compressores, equalizadores e amplificadores. Quando o guia menciona um "compressor de rack" (ex: Behringer MDX4600), refere-se a um equipamento externo à mesa que seria instalado nessa estrutura.
...
1. **Retorno** — Veja *Retorno / Monitor (áudio)*.
1. **Retroalimentação (feedback)** — Ciclo vicioso em que o som emitido por uma caixa é captado por um microfone próximo, amplificado novamente, captado de novo e assim por diante. O resultado é o apito agudo característico da microfonia. Controlado mantendo distância entre microfones e caixas e ajustando corretamente os volumes dos retornos.
...
1. **SIGNAL** — Luz verde presente em cada canal da mesa. Quando pisca, indica que há sinal de áudio chegando naquele canal. Se não acende, pode indicar microfone desligado, cabo desconectado ou GAIN muito baixo. É o primeiro indicador visual de que o canal está recebendo som.
1. **Sonorização** — O conjunto completo de equipamentos e técnicas usados para amplificar e distribuir o som num ambiente — inclui microfones, mesa de som, amplificadores, caixas de som, cabos e retornos. "Sonorização da igreja" é todo o sistema de som instalado.
