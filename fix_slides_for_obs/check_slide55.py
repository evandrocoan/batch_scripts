...
slide_orig = list(prs_orig.slides)[54]  # 0-indexed
slide_proc = list(prs_proc.slides)[54]

print('=== SLIDE 55 ORIGINAL ===')
...
for shape in slide_proc.shapes:
    if shape.has_text_frame and shape.text_frame.text.strip():
        ...
        if max_font > 0:
            text_size = processor.measure_multiline_text_size(text, font_name, max_font, available_width_pt - 20)
            if text_size:
                ...
                if text_size[1] > box_height:
                    print(f'  *** OVERFLOW by {text_size[1] - box_height:.0f}pt ***')
                else:
                    remaining = box_height - text_size[1]
                    usage_pct = (text_size[1] / box_height) * 100
